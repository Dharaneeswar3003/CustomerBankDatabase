...
public class CustomerTest {
    ...
    public void testDeposit() {
        double newDepositCheckAmt = 500.0;
        double newCheckBalance = customer.deposit( newDepositCheckAmt, today, Customer.CHECKING);
        assertEquals(1500.0, newCheckBalance, 0.01); 
        double newDepositWithdrawAmt = 300.0;
        double newSaveBalance = customer.deposit(newDepositWithdrawAmt, today, Customer.SAVING);
    ...
    public void testInvalidAmount() {
        ...
        double invalidDepositResult = customer.deposit(-100.0, today, Customer.CHECKING);
        assertEquals(-1, invalidDepositResult, 0.01);
        double invaliddepositResult = customer.deposit(-300.0, today, Customer.SAVING);
        assertEquals(-1, invaliddepositResult, 0.01);

        // Test invalid withdrawal amount
        double invalidwithdrawResult = customer.withdraw(-100.0, today, Customer.CHECKING);
        assertEquals(-1, invalidwithdrawResult, 0.01);
        double invalidWithdrawResult = customer.withdraw(-50.0, today, Customer.SAVING);
        assertEquals(-1, invalidWithdrawResult, 0.01);
    }
}
